...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (approved === undefined) {
      return NextResponse.json({ error: 'approved parameter is required' }, { status: 400 });
    }
    ...
    // 3. Create Notification & Push to owner
    if (salon.owner_id) {
      ...
      if (profile?.push_token && profile.push_token.startsWith('ExponentPushToken')) {
        try {
          ...
          console.error('Failed to send push notification to owner:', err.message);
        }
      }
    }
